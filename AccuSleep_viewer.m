function [message] = AccuSleep_viewer(EEG, EMG, SR, epochLen, userLabels, savepath)
    ...
    % EEG signal - 1 Minute
    G.A6a = axes('Units', 'Normalized', 'Position', [0.05 0.705, 0.87 .09]);
    G.A6a.Toolbar.Visible = 'off';
    ylabel(G.A6a, 'EEG1');
    
    % % EMG signal - 1 Minute
    G.A7a = axes('Units', 'Normalized', 'Position', [0.05 0.59 0.87 .09]);
    ...
    % EEG signal  - main
    G.A6 = axes('Units', 'Normalized', 'Position', [0.05 0.475 0.87 .09]);
