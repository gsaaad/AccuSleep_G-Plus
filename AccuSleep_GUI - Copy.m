...
% check if all labels are outside the range 1:6
if all(labels > 6 | labels < 1)
    animateBoxes(getappdata(handles.D,'outputIndicators'),0);
    disptext(handles, 'ERROR: At least some labels must be in the range 1:6.');
...
if sum(labels <= 6 | labels >= 1) * ts / 60 < 5
...
if isempty(calibrationData)
    animateBoxes(getappdata(handles.D,'outputIndicators'),0);
    disptext(handles, 'ength of label file does not match length');
